    ...
    protected function upload_file() {
        ...
        if(isset($_FILES['file_upload'])) {
            ...
            if(!$file_upload ) {
                ...
                $log_message = "ERROR: The file " . $this->sanitize_file_name( $_FILES['file_upload']['name'] ) . " for adnumber " . $this->post_array['adnumber'] . " failed to upload.";
                
                if(isset($_FILES['file_upload']['error']) && $_FILES['file_upload']['error'] != '') {
                    $log_message .= "ERROR: The  for adnumber " . $this->post_array['adnumber'] . " was " . $_FILES['file_upload']['error'];
                }
...
class BookingsXmlForMediaSpectrum extends PostData {
    ...
    private function _get_ms_run_date() {
        ...
        if(isset($this->post_array['proof_date']) && $this->post_array['proof_date'] != '') {
            $run_date = $this->post_array['proof_date'];
        } else {
            $run_date = $this->post_array['due_date'];
        }
        
        $ms_date = explode('/',$run_date);
...
class CreativeHTMLBrief extends PostData {
    ...
    protected function create_html() {
        ...
        $uploaded_file = $this->_get_uploaded_file_name();
        
        if($uploaded_file != '') {
            $html .= "<tr>";
            $html .= "<td><b>File Attachment</b></td>";
            $html .= "<td>" . $uploaded_file . "</td>";
            $html .= "</tr>";
        }
    ...
    private function _get_uploaded_file_name() {
        $uploaded_file = parent::upload_file();
